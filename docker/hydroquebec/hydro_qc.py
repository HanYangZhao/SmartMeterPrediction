...
class HydroQuebecClient(object):
    ...
    def fetch_data(self):
        ...
        for contract, contract_url in contracts.items():
            ...
            yesterday_hourly_data = yield from self._get_hourly_data(day_date, p_p_id)
            two_days_ago = datetime.date.today() - datetime.timedelta(days=2)
            day_date = two_days_ago.strftime("%Y-%m-%d")
            two_days_ago_hourly_data = yield from self._get_hourly_data(day_date, p_p_id)
            #hourly_data = two_days_ago_hourly_data.append(yesterday_hourly_data)
            # Get Annual data
            try:
            ...
            # Hourly
            if yesterday_hourly_data:
                contract_data['yesterday_hourly_consumption'] = yesterday_hourly_data
                contract_data['two_days_ago_hourly_consumption'] = two_days_ago_hourly_data
            # Add contract
